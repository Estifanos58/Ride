  ...
  const onVerifyPress = async () => {
    ...
    try {
      ...
      if (signUpAttempt.status === 'complete') {
        ...
        setVerification({
          ...verification, state: 'failed', error: 'Verification faild'
        })
      ...
      // See https://clerk.com/docs/custom-flows/error-handling
      setVerification({
        ...verification, state: 'failed', error: err.errors[0].longMessage
      })
      // for more info on error handling
      console.error(JSON.stringify(err, null, 2))
    }
      ...
      <View className="flex-1 bg-white">
        ...
        <ReactNativeModal isVisible={verification.state== "pending"} onModalHide={()=> setVerification({...verification, state:'success'})}>
          <View className="bg-white px-7 py-9 rounded-2xl min-h-[300px]">
           ...
           <InputField label="Code" icon={icons.lock} placeholder="12345" value={verification.code} keyboardType="numeric" onChangeText={(value)=> setVerification({...verification, code:value})}/>
           {
            verification.error && (
        ...
        <ReactNativeModal isVisible={verification.state== "success"}>
          <View className="bg-white px-7 py-9 rounded-2xl min-h-[300px]">
            <Image source={images.check} className="w-[100px] h-[100px] mx-auto my-5"/>
